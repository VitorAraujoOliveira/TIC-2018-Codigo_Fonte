...
class insereAtividade(Resource):
	def post(self,nome_atividade,local_atividade,data_atividade,categoria,fornecido,descricao,imagem):
		...
		image  = plt.imsave('C:\\Users\\vitor\\Desktop\\TIC-2018\\TESTE.png', np.array(imagem))
...
class verificaEvento(Resource):
	def get(self,id_aluno):
		conn = psycopg2.connect(host="localhost",database="DaHora", user="postgres", password="1234")
		cur = conn.cursor()

		query = cur.execute("select nome,data_inicio,horas_dia,numero_atividades,local_evento from evento where data_inicio is not null")
		keys = ['nome','data_inicio','horas_dia','nro_atividades','local_evento']
...
api.add_resource(verificaEvento,'/eventos/<id_aluno>')
